...
The policy implications are urgent because atrophy operates on generational timescales that exceed typical planning horizons. By the time expertise loss becomes visible through system failures, the knowledge required for recovery may already be unavailable. This creates what we term the "atrophy trap"—the point at which society lacks the expertise to recognize, diagnose, or remediate its own capability loss.
...
<Mermaid client:load chart={`flowchart TD
    subgraph "Level 1: Individual (1-5 years)"
        A[AI assists Task A] --> B[Practice of Skill A declines]
        B --> C[Skill A proficiency drops]
        C --> D[Cannot perform dependent Task B]
        D --> E[Skill B atrophies]
        E --> F[Increased AI dependence]
        F --> A
    end

    subgraph "Level 2: Institutional (5-15 years)"
        G[Individual expertise declines] --> H[Training quality degrades]
        H --> I[New hires less capable]
        I --> J[Institutional knowledge gaps]
        J --> K[Documentation assumes AI]
        K --> L[Recovery becomes impossible]
    end

    subgraph "Level 3: Generational (15-40 years)"
        M[Gen 1 has skills, uses AI] --> N[Gen 2 learns with AI from start]
        N --> O[Gen 2 cannot train Gen 3]
        O --> P[Gen 3 never learns fundamentals]
        P --> Q[Knowledge effectively lost]
    end

    F --> G
    L --> M
    Q --> R[Civilizational capability loss]
...
<Mermaid client:load chart={`flowchart TD
    subgraph "Stage 1: Image Interpretation (Years 1-3)"
        A1[AI reads X-rays/CTs/MRIs] --> A2[Radiologists confirm AI findings]
        A2 --> A3[Pattern recognition practice declines]
        A3 --> A4[Subtle abnormality detection falls 30-50%]
    end

    subgraph "Stage 2: Clinical Correlation (Years 3-7)"
        A4 --> B1[Cannot correlate findings with symptoms]
        B1 --> B2[Miss atypical presentations]
        B2 --> B3[Novel disease recognition drops 40-60%]
        B3 --> B4[Over-reliance on AI differential diagnosis]
    end

    subgraph "Stage 3: Knowledge Transmission (Years 7-15)"
        B4 --> C1[Cannot teach clinical reasoning]
        C1 --> C2[Residents learn AI-mediated diagnosis]
        C2 --> C3[Training quality degrades 50-70%]
        C3 --> C4[Next generation cannot supervise AI]
    end

    subgraph "Stage 4: System Fragility (Years 15+)"
        C4 --> D1[AI errors go undetected]
        D1 --> D2[Misdiagnosis rates rise 20-40%]
        D2 --> D3[System fails during AI outages]
        D3 --> D4[Cannot function without AI]
    end
...
The fundamental challenge is that atrophy costs are diffuse and long-term while AI adoption benefits are immediate and concentrated. This creates a collective action problem where individually rational choices lead to collectively harmful outcomes. Without coordinated intervention, the default trajectory leads toward the "unmanaged cascade" scenario with 35% probability—resulting in skill levels of 20-35% by 2050 and very limited recovery options.
