...
<Mermaid client:load chart={`flowchart TD
    subgraph "Core Feedback Loops"
        A[Market Leader] --> B[More Users]
        B --> C[More Data]
        C --> D[Better Models]
        D --> A
        ...
        A --> G[Prestige & Culture]
        G --> H[Top Talent]
        H --> D

        A --> I[Developer Ecosystem]
        I --> J[More Applications]
        J --> B

        A --> K[IP & Partnerships]
        K --> L[Barriers to Entry]
        L --> A
    end
...
<Mermaid client:load chart={`flowchart TD
    subgraph High_Leverage[High Leverage - Difficult]
        I1[Antitrust enforcement] --> O1[Prevents lock-in]
        I2[Public compute infrastructure] --> O2[Democratizes access]
    end

    subgraph Medium_Leverage[Medium Leverage - Moderate Difficulty]
        I3[Open-source funding] --> O3[Creates alternatives]
        I4[Data portability mandates] --> O4[Reduces flywheel]
        I5[Interoperability requirements] --> O5[Lowers switching costs]
    end

    subgraph Lower_Leverage[Lower Leverage - Easier]
        I6[Talent development programs] --> O6[Expands talent pool]
        I7[Redistribution mechanisms] --> O7[Addresses inequality]
    end
