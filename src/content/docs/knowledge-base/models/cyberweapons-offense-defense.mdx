...
```mermaid
flowchart TD
    subgraph "Structural Asymmetries"
        A[Target Selection<br/>Attackers choose weakest link] --> OA[Offense Advantage]
        B[Disclosure Timing<br/>Attackers stockpile 0-days] --> OA
        C[Success Threshold<br/>One breach vs 100% defense] --> OA
        D[Data Position<br/>Defenders know own systems] --> DA[Defense Advantage]
        E[Terrain Control<br/>Defenders set architecture] --> DA
    end

    subgraph "AI Multipliers"
        OA --> F{AI Capability<br/>Multiplication}
        DA --> F
        F --> G[Vulnerability Discovery ×1.5-2.0]
        F --> H[Attack Automation ×2.0-3.0]
        F --> I[Detection Speed ×10-100]
        F --> J[Response Time ×50-200]
    end

    subgraph "Net Balance"
        G --> K[Current State:<br/>Offense +30-70%]
        H --> K
        I --> K
        J --> K
        K --> L{Trajectory}
...
Where:
- $B_{OD}$ = Offense-defense balance ratio (>1 favors offense, <1 favors defense)
