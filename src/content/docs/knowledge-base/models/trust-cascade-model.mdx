...
flowchart TD
    subgraph Initial_Shock["Initial Shock Event"]
        A[AI Deepfake Scandal or Major Institutional Failure]
    end

    subgraph Primary_Impact["Primary Impact"]
        B[Media Trust Falls Below 30%]
    end

    subgraph Cascade_Phase["Cascade Propagation"]
        C[Science Claims Cannot Be Verified]
        D[Government Communication Loses Credibility]
        E[Legal Evidence Becomes Unreliable]
    end

    subgraph Threshold_Check["Critical Threshold Check"]
        F{Trust Level > 35%?}
    end

    subgraph Recovery["Recovery Possible"]
        G[Institutional Resilience Activated]
    end

    subgraph Collapse["Cascade Collapse"]
        H[Cross-Validation Network Fails]
        I[All Institutions Below Critical Threshold]
        J[System-Wide Trust Collapse]
    end
...
<Mermaid client:load chart={`
stateDiagram-v2
    [*] --> StableHighTrust: Initial State

    StableHighTrust: Stable High Trust (T > 0.5)
    StableHighTrust: Self-reinforcing validation
    StableHighTrust: Strong recovery capacity

    VulnerableState: Vulnerable State (0.35-0.5)
    VulnerableState: Validation weakening
    VulnerableState: Cascade risk emerging

    CollapsedState: Collapsed State (0.15-0.35)
    CollapsedState: Cannot validate others
    CollapsedState: Rebuilding seen as manipulation

    CompleteCollapse: Complete Collapse (T < 0.15)
    CompleteCollapse: Institution non-functional
    CompleteCollapse: Recovery may be impossible

    StableHighTrust --> VulnerableState: Gradual erosion (years to decades)
    VulnerableState --> StableHighTrust: Active intervention (years)
    VulnerableState --> CollapsedState: Shock event or SUDDEN CASCADE (weeks to months)
    CollapsedState --> VulnerableState: Massive intervention (decades)
    CollapsedState --> CompleteCollapse: Continued attacks (months to years)
    CompleteCollapse --> [*]: Generational replacement or permanent loss
`} />
