...
flowchart TD
    subgraph Initial_Shock["Initial Shock Event"]
        A[AI Deepfake Scandal<br/>or Major Institutional Failure]
    end

    subgraph Primary_Impact["Primary Impact"]
        B[Media Trust<br/>Falls Below 30%]
    end

    subgraph Cascade_Phase["Cascade Propagation"]
        C[Science Claims<br/>Cannot Be Verified]
        D[Government Communication<br/>Loses Credibility]
        E[Legal Evidence<br/>Becomes Unreliable]
    end
    ...
    subgraph Recovery["Recovery Possible"]
        G[Institutional<br/>Resilience Activated]
    end

    subgraph Collapse["Cascade Collapse"]
        H[Cross-Validation<br/>Network Fails]
        I[All Institutions<br/>Below Critical Threshold]
        J[System-Wide<br/>Trust Collapse]
    end
...
stateDiagram-v2
    ...
    StableHighTrust --> VulnerableState: Gradual erosion\n(years to decades)
    VulnerableState --> StableHighTrust: Active intervention\n(years)
    VulnerableState --> CollapsedState: Shock event or\nSUDDEN CASCADE\n(weeks to months)
    CollapsedState --> VulnerableState: Massive intervention\n(decades)
    CollapsedState --> CompleteCollapse: Continued attacks\n(months to years)
    CompleteCollapse --> [*]: Generational replacement\nor permanent loss
```
