...
flowchart TD
    subgraph Leading["Leading Indicators (Predictive)"]
        CAP[Capability Signals<br/>Benchmark improvements]
        BEH[Behavioral Signals<br/>System behaviors in eval]
        RES[Research Signals<br/>Publications, breakthroughs]
    end

    subgraph Lagging["Lagging Indicators (Confirmatory)"]
        INC[Incident Signals<br/>Real-world events]
        SOC[Social Signals<br/>Institutional responses]
    end
    ...
    CAP --> TRP{Tripwire<br/>Threshold}
    BEH --> TRP
    INC --> TRP
    TRP --> |"Crossed"| ACT[Predetermined<br/>Response]
    TRP --> |"Approaching"| MON[Heightened<br/>Monitoring]
```
