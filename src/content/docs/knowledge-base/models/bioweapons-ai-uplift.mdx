...
```mermaid
flowchart TD
    subgraph "Information Access Layer"
        A[Scientific Literature] --> D{Attacker Knowledge<br/>Acquisition}
        B[Internet Sources] --> D
        C[AI Systems] --> D
    end

    subgraph "Capability Development"
        D --> E{Technical<br/>Planning}
        E --> F{Resource<br/>Acquisition}
        F --> G{Laboratory<br/>Synthesis}
    end

    subgraph "Deployment"
        G --> H{Biosecurity<br/>Evasion}
        H --> I{Weaponization<br/>& Delivery}
    end

    subgraph "AI Uplift Points"
