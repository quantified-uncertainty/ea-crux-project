...
graph TD
    A[Safety Intervention] --> B{Relationship Type}
    B -->|Competitive| C[Slows Capability Development]
    B -->|Complementary| D[Enhances Capability Development]
    B -->|Orthogonal| E[Independent of Capabilities]

    C --> F[Short-term Cost]
    D --> G[Long-term Benefit]
    E --> H[Parallel Progress]

    F --> I{Time Horizon}
    G --> I
    H --> I

    I -->|Short-term| J[Resources Finite]
    I -->|Medium-term| K[Mixed Effects]
    I -->|Long-term| L[Often Complementary]
...
<Mermaid client:load chart={`
flowchart LR
    A[Lab Investment Decision] --> B{Resource Allocation}

    B --> C[Safety Investment]
    B --> D[Capability Investment]

    C --> E[Short-term: Reduced Capability]
    C --> F[Long-term: Enables Deployment]

    D --> G[Short-term: Higher Capability]
    D --> H[Long-term: Risk of Shutdown]

    E --> I{Racing Dynamics}
    G --> I

    I -->|Strong Competition| J[Pressure to Minimize Safety]
    I -->|Coordination| K[Safety Investment Viable]

    F --> L[Sustained Development]
    H --> M[Potential Catastrophe]
